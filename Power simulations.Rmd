...
Next, let's regenerate the data. Since we're reusing the same code, let's make functions out of the code. And then just call them in one pipe:

```{r, include=FALSE}
my.generate = function(Correct.mu.cond, RT.mu.cond) {
  crossing(
    exp = 1:nexp,
    condition = c("A","B","C"),
    trial = 1:ntrial,
...
```{r Type I}
d0 = 
  my.generate(RT.mu.cond = RT.mu.cond, Correct.mu.cond = Correct.mu.cond) %>%
  my.sample() 
